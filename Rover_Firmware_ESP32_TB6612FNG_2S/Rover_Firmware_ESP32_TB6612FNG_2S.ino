...
class BattInfoCallbacks: public BLECharacteristicCallbacks {
    void onRead(BLECharacteristic *pCharacteristic) {
      char BattLevel[10];
      sprintf(BattLevel,"%4.0f",calculateBattLevel(VBAT, 6.4f, 8.4f));
...
class NavigationCallbacks: public BLECharacteristicCallbacks {
    void onWrite(BLECharacteristic *pCharacteristic) {
      ...
      acc += value[6];
      acc += value[7];
      float accVal = (float)acc.toInt();
      accVal = accVal*(5.0f/VBAT);
      ledcWrite(1, (int)accVal);
      Serial.println(acc);
...
void loop() {
  VBAT = ((127.0f * 2.0f)/100.0f) * 3.30f * float(analogRead(BattSense)) / 4095.0f;  // LiPo battery
